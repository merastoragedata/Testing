...
// ======================================================================
// FAST-LOAD BAY FILES
// Loading all 45,593 settings on every boot was the main source of slowness.
// _Settings remains the master (so "Database (auto)" is unaffected); these files
// are DERIVED fast-read caches:
//     Data/index.json      tiny — bays, relay names/kinds/counts/vitals.
//                          Enough to draw Overview, SLD and Bay Register with
//                          NO setting values at all (~60 KB instead of ~4 MB).
//     Data/bay-<id>.json   one bay's full settings, fetched only when opened.
// getBaysData accepts an ARRAY of bay ids so several bays come back in ONE
// request — Apps Script's cost is per-call overhead, not payload size.
// Rebuild them with 'buildBayFiles' after any database sync.
// ======================================================================
...
function getDataFolder_() {
  const root = getRootFolder_();
  const it = root.getFoldersByName(DATA_FOLDER_NAME);
  if (it.hasNext()) return it.next();
  return root.createFolder(DATA_FOLDER_NAME);
}
function writeJsonFile_(folder, name, obj) {
  const txt = JSON.stringify(obj);
  const it = folder.getFilesByName(name);
  if (it.hasNext()) { const f = it.next(); f.setContent(txt); return f; }
  return folder.createFile(name, txt, 'application/json');
}
...
function bayFileName_(bayId) { return 'bay-' + sanitizeId_(bayId) + '.json'; }

function buildBayFiles_() {
  const ss = getSS_();
  const bSh = ss.getSheetByName(BAYS_SHEET);
  const rSh = ss.getSheetByName(RELAYS_SHEET);
  const sSh = ss.getSheetByName(SETTINGS_SHEET);
  if (!bSh || !rSh || !sSh) return { ok: false, error: 'Database not synced yet — run "Sync full data to Storage" first.' };
  ...
  // group settings: relayId -> group -> [rows]
  const byRelay = {};
  settingRows.forEach(function (s) {
    const rid = s[0], grp = s[1];
    if (!byRelay[rid]) byRelay[rid] = {};
    if (!byRelay[rid][grp]) byRelay[rid][grp] = [];
    const o = { t: s[2], v: s[3], u: s[4], s: s[5], r: s[6] };
    if (s[7]) o.desc = s[7];
    if (s[8]) o.vm = s[8];
    byRelay[rid][grp].push(o);
  });
  const relaysByBay = {};
  relayRows.forEach(function (r) {
    if (!relaysByBay[r[1]]) relaysByBay[r[1]] = [];
    relaysByBay[r[1]].push(r);
  });

  const folder = getDataFolder_();
  const idxBays = [], idxZones = [];
  let written = 0;

  bayRows.forEach(function (row) {
    const id = row[0], isZone = row[7] === true || row[7] === 'TRUE';
    const rows = relaysByBay[id] || [];

    // --- index entry: metadata only, NO setting values ---
    const relMeta = rows.map(function (r) {
      let vitals = []; try { vitals = JSON.parse(r[8] || '[]'); } catch (e) {}
      return { id: r[0], name: r[2], file: r[3], family: r[4], model: r[5], kind: r[6], n_settings: r[7], vitals: vitals };
    });
    const entry = isZone
      ? { id: id, name: row[2], voltage: row[3], relays: relMeta }
      : { id: id, num: row[1], name: row[2], voltage: row[3], scheme: row[4], dia: row[5] || null, pos: row[6] || null, relays: relMeta };
    (isZone ? idxZones : idxBays).push(entry);

    // --- per-bay file: the heavy part ---
    const full = rows.map(function (r) {
      const gm = byRelay[r[0]] || {};
      let vitals = []; try { vitals = JSON.parse(r[8] || '[]'); } catch (e) {}
      return { id: r[0], name: r[2], file: r[3], family: r[4], model: r[5], kind: r[6], n_settings: r[7],
        vitals: vitals, groups: Object.keys(gm).map(function (g) { return { group: g, settings: gm[g] }; }) };
    });
    writeJsonFile_(folder, bayFileName_(id), { bayId: id, relays: full });
    written++;
  });

  const index = {
    name: meta.name || '', ssCode: meta.ssCode || '', docDate: meta.docDate || '',
    stats: { bays: Number(meta.statsBays) || idxBays.length, relays: Number(meta.statsRelays) || relayRows.length,
             settings: Number(meta.statsSettings) || settingRows.length },
    bays: idxBays, busZones: idxZones,
    syncedAt: meta.syncedAt || '', builtAt: new Date().toISOString()
  };
  writeJsonFile_(folder, INDEX_FILE, index);
  return { ok: true, bayFiles: written, indexBays: idxBays.length, indexZones: idxZones.length, settings: settingRows.length };
}

function readIndex_() {
  const idx = readJsonFile_(getDataFolder_(), INDEX_FILE);
  if (!idx) return { ok: false, error: 'Fast-load index not built yet — run "Rebuild fast-load files" in Admin.' };
  return Object.assign({ ok: true }, idx);
}

// Several bays in ONE round trip.
function readBaysData_(bayIds) {
  const folder = getDataFolder_();
  const out = {};
  (bayIds || []).forEach(function (id) {
    const j = readJsonFile_(folder, bayFileName_(id));
    if (j) out[id] = j.relays || [];
  });
  return { ok: true, bays: out };
}
...
function doGet(e) {
  try {
    ...
    if (action === 'getIndex') return jsonOut_(readIndex_());
    if (action === 'getBay') return jsonOut_(readBaysData_([e.parameter.bayId]));
...
function doPost(e) {
  try {
    ...
    if (action === 'bootstrap') {
      ...
      // Regenerate the fast-load index + per-bay files straight away, so they can never
      // drift from the database that was just written.
      let fast = {};
      try { fast = buildBayFiles_(); } catch (e) { fast = { ok: false, error: String(e) }; }
      return jsonOut_(Object.assign({ ok: true }, result, { fastLoad: fast }));
    }
    if (action === 'version') return jsonOut_({ ok: true, backendVersion: BACKEND_VERSION });
    if (action === 'getFullData') return jsonOut_(readFullDatabase_());
    if (action === 'getSld') return jsonOut_(readSld_());
    ...
    if (action === 'getUserDataMeta') return jsonOut_(readUserDataMeta_());
    if (action === 'getIndex') return jsonOut_(readIndex_());
    if (action === 'getBaysData') return jsonOut_(readBaysData_(body.bayIds || []));   // batched: many bays, one call
    if (action === 'buildBayFiles') return jsonOut_(buildBayFiles_());
    ...
    // Bulk write from an Excel upload. The "Database (auto)" tab is script-owned and is
    // NEVER a valid target — writing there would destroy the filtered database view (this
    // happened once). Uploads always land in their own tab, placed AFTER the database tab.
    if (action === 'save') {
      let sh = body.blockId ? tabByGid_(ss, body.blockId) : null;
      if (sh && sh.getName() === DB_TAB_NAME) sh = null;          // refuse: protected tab
      if (!sh) {
        const wanted = String(body.name || 'Uploaded').slice(0, 90);
        let nm = wanted, i = 2;
        while (ss.getSheetByName(nm)) { nm = wanted.slice(0, 86) + ' ' + i; i++; }
        sh = ss.insertSheet(nm, 1);                                // index 1 = right after Database (auto)
      }
      writeOverviewTab_(sh, body.rows || []);
      return jsonOut_({ ok: true, blockId: String(sh.getSheetId()), name: sh.getName(),
        sheetUrl: sheetUrlFor_(ss, sh.getSheetId()), blocks: listOverviewTabs_(ss) });
    }
    ...
    if (action === 'deleteTab') {
      ...
      if (sh && sh.getName() === DB_TAB_NAME) return jsonOut_({ ok: false, error: 'The "Database (auto)" tab is generated from the settings database and cannot be deleted here.' });
